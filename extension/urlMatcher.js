/**
 * urlMatcher.js — Wildcard base domain normalizer (content script).
 *
 * Loaded as a content script BEFORE content.js. Provides getBaseDomain and
 * domainMatches to other content scripts (inline-overlay.js, save-prompt.js)
 * via a Symbol-keyed namespace on window, so page scripts cannot discover
 * or override these functions (E8 fix: no global namespace pollution).
 *
 * Previously, these were exposed as window.getBaseDomain / window.domainMatches
 * (string-keyed globals), which page scripts could detect, override, or use
 * as fingerprinting signals to identify the extension. Now they are stored
 * under a Symbol key, which:
 *   - Does not appear in Object.keys() or for...in enumeration
 *   - Cannot be accidentally discovered by page scripts
 *   - Cannot be overridden without knowing the exact Symbol value
 *   - Is shared across the extension's content scripts via Symbol.for()
 *
 * Content scripts that can't use ES modules in MV3 access these via:
 *   const _svUtils = Symbol.for('SecureVault.domainUtils');
 *   const utils = window[_svUtils];
 *   if (utils) utils.getBaseDomain(url);
 *
 * If urlMatcher.js fails to load, other scripts fall back to sending
 * chrome.runtime.sendMessage to background.js (async, but safe).
 *
 * E13: The PUBLIC_SUFFIXES_TWO_PART set has been expanded to ~150 entries
 * covering the most common two-part TLDs. The full Mozilla PSL has ~8,000
 * entries; this covers the vast majority of practical use. For entries not
 * in the list, the algorithm falls back to simple last-two-part extraction.
 */
...
const PUBLIC_SUFFIXES_TWO_PART = new Set([
  ...
  // Isle of Man / Channel Islands (already listed above)
...
function getBaseDomain(url) {
  ...
  // the port colon. If port stripping happens first, indexOf(':') finds
  // the auth colon and splits at the wrong position.
  // v10.9 FIX: Moved auth stripping BEFORE port stripping.
  const at = s.indexOf('@');
...
function domainMatches(urlA, urlB) {
...
// ─── E8: Store utilities under a Symbol key (no global namespace pollution) ───
// Previously: window.getBaseDomain / window.domainMatches (string-keyed globals)
// that page scripts could discover, override, or use as fingerprinting signals.
// Now: stored under Symbol.for('SecureVault.domainUtils'), which:
//   - Does not appear in Object.keys() or for...in enumeration
//   - Cannot be discovered by page scripts scanning window properties
//   - Cannot be overridden without knowing the exact Symbol
//   - Is accessible to our other content scripts via Symbol.for()
//
// Other content scripts access these utilities via:
//   const _svUtils = Symbol.for('SecureVault.domainUtils');
//   const utils = window[_svUtils];
//   if (utils) { const base = utils.getBaseDomain(url); }
//
// If urlMatcher.js hasn't loaded yet (or failed to load), the fallback
// is to send chrome.runtime.sendMessage to background.js (async).

const _svUtils = Symbol.for('SecureVault.domainUtils');

if (typeof window !== 'undefined') {
  window[_svUtils] = { getBaseDomain, domainMatches };
}
