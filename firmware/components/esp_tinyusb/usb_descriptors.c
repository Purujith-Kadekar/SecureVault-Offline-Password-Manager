...
uint8_t const descriptor_fs_cfg_default[] = {
    ...
    // CHANGED: added the two trailing args (_ep_notif_interval=16,
    // _capability=0) the macro now requires. esp_tinyusb's original call
    // (9 args) was written against an older tinyusb version; the
    // vendored tinyusb (components/tinyusb, pulled from current master
    // since the exact pinned version wasn't fetchable) added
    // _ep_notif_interval and _capability to this macro since then.
    // capability=0 means no optional NCM features advertised (matches
    // what network_manager.cpp actually implements -- no checksum
    // offload/statistics support claimed). interval=16 is a standard,
    // safe notification-endpoint polling interval, not safety-critical.
    TUD_CDC_NCM_DESCRIPTOR(ITF_NUM_NET, STRID_NET_INTERFACE, STRID_MAC, (0x80 | EPNUM_NET_NOTIF), 64, EPNUM_NET_DATA, (0x80 | EPNUM_NET_DATA), 64, CFG_TUD_NET_MTU, 16, 0),
#endif
...
#if CFG_TUD_NCM
    // Interface number, description string index, MAC address string index, EP notification address and size, EP data address (out, in), and size, max segment size.
    // Same fix as the full-speed descriptor above -- see that comment.
    TUD_CDC_NCM_DESCRIPTOR(ITF_NUM_NET, STRID_NET_INTERFACE, STRID_MAC, (0x80 | EPNUM_NET_NOTIF), 64, EPNUM_NET_DATA, (0x80 | EPNUM_NET_DATA), 512, CFG_TUD_NET_MTU, 16, 0),
#endif
